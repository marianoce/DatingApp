    ...
    public class MessageRepository : IMessageRepository
    {
        ...
        public async Task<IEnumerable<MessageDto>> GetMessageThread(string currentUserName, string recipientUserName)
        {
            var messages = await _context.Messages
                .Include(u => u.Sender).ThenInclude(f => f.Photos)
                .Include(u => u.Recipient).ThenInclude(f => f.Photos)
                .Where(p => (p.Recipient.UserName == currentUserName && p.Sender.UserName == recipientUserName && !p.RecipientDeleted)
                        || (p.Recipient.UserName == recipientUserName && p.Sender.UserName == currentUserName && !p.SenderDeleted))
